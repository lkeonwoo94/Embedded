...
#if ((GTM_SFR_RESET_ENABLE == STD_ON) || (GTM_DEINIT_API_ENABLE == STD_ON))
...
void Gtm_IsrAtomModule(uint8 ModuleNo, uint8 ChannelNumber)
{
  ...
  */
  ...
  #if (GTM_MCUSAFETY_ENABLE == STD_ON)
  boolean IsAtomChSpuriousInterrupt = TRUE;
  #endif
  ...
  */
  ...
  do
  {
    ...
    if(Gtm_lGetIntSource(IrqStatus, ChanIndex))
    {
      ...
      #if (GTM_MCUSAFETY_ENABLE == STD_ON)
      /* current Interrupt is not a spurious interrupt*/
      IsAtomChSpuriousInterrupt = FALSE;
      #endif
    ...
    #if (GTM_MCUSAFETY_ENABLE == STD_ON)
    /* if current interrupt is a spurious interrupt then report safety error */
    if(IsAtomChSpuriousInterrupt == TRUE)
    {
       SafeMcal_ReportError ((uint16)GTM_MCU_MODULE_ID,GTM_MCU_MODULE_INSTANCE,
                     GTM_SID_ATOM_ISR,GTM_E_SPURIOUS_INTRPT);
    }
    else
    {
      /* Do Nothing */
...
IFX_LOCAL_INLINE void Gtm_lAruConfigure(void)
{
                  ...
                  GTM_ARU_NOT_CONFIGURED) == GTM_ZERO_U)
  {
    ...
    GTM_ARU_IRQ_MODE.U = (unsigned_int)                                      \
                    (((uint32)Gtm_kConfigPtr->GtmModuleConfigPtr->GtmAruConfig\
                                   & GTM_GET_MODE_VAL_8) >> GTM_SHIFT_TO_LSB_8);
    GTM_ARU_IRQ_EN.U = (unsigned_int)  \
                 ((uint32)Gtm_kConfigPtr->GtmModuleConfigPtr->GtmAruConfig    \
                                                         & GTM_GET_ARU_IRQ_VAL);

    /* SRN Enable */
    SRC_GTMARUIRQ0.U |= (unsigned_int)(GTM_BIT_SET << GTM_BIT_SRE);
    SRC_GTMARUIRQ1.U |= (unsigned_int)(GTM_BIT_SET << GTM_BIT_SRE);
    SRC_GTMARUIRQ2.U |= (unsigned_int)(GTM_BIT_SET << GTM_BIT_SRE);
  }
}
/*******************************************************************************
           ...
           Count < (GTM_NO_OF_FIFO_CHANNELS * GTM_NO_OF_FIFO_MODULES); Count++)
    {
     ...
     FifoConfigPtr =Gtm_kConfigPtr->GtmModuleConfigPtr->GtmFifoConfigPtr[Count];
      ...
      if(FifoConfigPtr != NULL_PTR)
      {
        ...
        FifoChannelPtr->IRQ_EN.U  = (unsigned_int)                     \
                  ((uint32)FifoConfigPtr->FifoIrqEnable & GTM_GET_FIFO_IRQ_VAL);
        /* Initialize the Irq Mode Register  */


        FifoChannelPtr->IRQ_MODE.U = (unsigned_int)  \
               (((uint32)FifoConfigPtr->FifoIrqEnable & GTM_FIFO_DMA_MASK)  \
  ...
  for(MajorCnt = GTM_ZERO_U; MajorCnt <  MajorCountLoopLimit; MajorCnt++)
  {
    for(MinorCnt = GTM_ZERO_U; (MinorCnt < GTM_BITS_IN_U32); MinorCnt++)
    {
     ...
     PBConfigStructure and is within allowed range.*/
        ...
        TimInScrRegPtr = &(((*(Ifx_GTM_TIMx*)(volatile void *)(MODULE_GTM.TIM)).
        ...
        if(TimConfigPtr->TimUsage == GTM_DRIVER_COMPLEX)
        {
          ...
          MODULE_GTM.TIM_AUX_IN_SRC[ModuleNo].U  = (unsigned_int)              \
     ...
     PBConfigStructure and is within allowed range.*/
        ...
        if(TomConfigPtr->GtmTomTgcConfigGrpPtr != NULL_PTR)
        {
          ...
          TomTgcRegPtr->OUTEN_STAT.U = \
           (unsigned_int)(TomConfigPtr->GtmTomTgcConfigGrpPtr->GtmTomOutenStat);
...
IFX_LOCAL_INLINE void Gtm_lAtomHostTrigger(void)
{
  ...
  for(MajorCnt = GTM_ZERO_U; MajorCnt < GTM_NO_OF_ATOM_MODULES; MajorCnt++)
  {
    ...
    {
      /* Invoke an Host Trigger */
      AtomAgcRegPtr->AGC.GLB_CTRL.U |= (unsigned_int)GTM_BIT_SET;
    }
  }
}
...
IFX_LOCAL_INLINE void Gtm_lAtomAgcConfigure(void)
{
 ...
 Permitted for special function registers.*/
        ...
        if(AtomConfigPtr->GtmAtomAgcConfigGrpPtr != NULL_PTR)
        {
          ...
          AtomAgcRegPtr->AGC_GLB_CTRL.U   =  (unsigned_int)\
...
IFX_LOCAL_INLINE void Gtm_lTbuConfig(void)
{
 ...
 Permitted for special function registers.*/
    ...
    for(Count =GTM_ZERO_U; Count < GTM_NO_OF_TBU_CHANNELS; Count++)
    {
     ...
     GTM_TBU_CHEN.U = (unsigned_int)(GTM_BIT_SET << (GTM_BITS_PER_TBU * Count));
            ...
            Count < (GTM_NO_OF_F2A_CHANNELS * GTM_NO_OF_F2A_MODULES); Count++)
    {
     ...
     F2aConfigPtr=&(Gtm_kConfigPtr->GtmModuleConfigPtr->GtmF2aConfigPtr[Count]);
      ...
      if(Gtm_lGetUnused8(F2aConfigPtr->F2aChannelConfig) == (boolean)FALSE)
      {
        ...
        F2AChannelPtr->ENABLE.U = (unsigned_int)(GTM_BIT_SET <<               \
        ...
        if(Gtm_lGetBit8(F2aConfigPtr->F2aChannelConfig,GTM_F2A_CONFIGURED) ==
          GTM_ONE_U)
        {
          F2AChannelPtr->ENABLE.U = (unsigned_int)\
      ...
      if(Gtm_lGetUnused8(McsConfigPtr->McsControl) == (boolean)FALSE)
      {
            ...
            LoopCount++)
        {
         ...
         ((*(Ifx_GTM_MCSx*)(volatile void *)(MODULE_GTM.MCS)).CH_MCS[Count].
          ...
          CH_MCS[Count].CH[LoopCount].CH_IRQ_MODE.U) = (unsigned_int)
           (((uint32)(McsConfigPtr->McsChannelConfig[LoopCount]) \
                                   & GTM_GET_MODE_VAL_8) >> GTM_SHIFT_TO_LSB_8);

          ((*(Ifx_GTM_MCSx*)(volatile void *)(MODULE_GTM.MCS)).CH_MCS[Count].\
           CH[LoopCount].CH_IRQ_EN.U)  = (unsigned_int)(((uint32)             \
           (McsConfigPtr->McsChannelConfig[LoopCount])& GTM_GET_MCS_IRQ_VAL)  \
                                                                 >> GTM_ONE_U);

          ((*(Ifx_GTM_MCSx*)(volatile void *)(MODULE_GTM.MCS)).
           CH_MCS[Count].CH[LoopCount].CH_EIRQ_EN.U) = (unsigned_int)         \
          ...
          if(Gtm_lGetBit8(McsConfigPtr->McsChannelConfig[LoopCount],GTM_ZERO_U))
          {
            ...
            ((*(Ifx_GTM_MCSx*)(volatile void *)(MODULE_GTM.MCS)).
            CH_MCS[Count].CH[LoopCount].CH_CTRL.U) = (unsigned_int)           \
...
IFX_LOCAL_INLINE void Gtm_lDpllConfig(void)
{
  ...
  if(DpllConfigPtr != NULL_PTR)
  {
    ...
    GTM_DPLL_CTRL_1.U &= (unsigned_int)(~GTM_DPLL_ENABLE);
    GTM_DPLL_CTRL_0.U = DpllConfigPtr->DpllCtrl0;
    /* Write all information except DEN */
    GTM_DPLL_CTRL_1.U = (unsigned_int)  \
                     (DpllConfigPtr->DpllCtrl1 & (uint32)(~GTM_DPLL_ENABLE));
    ...
    GTM_DPLL_CTRL_2.U = (unsigned_int)  \
                          (((uint32)DpllConfigPtr->DpllCtrl2) << GTM_EIGHT_U);
    GTM_DPLL_CTRL_3.U = (unsigned_int)  \
                          (((uint32)DpllConfigPtr->DpllCtrl3) << GTM_EIGHT_U);
    GTM_DPLL_CTRL_4.U = (unsigned_int)  \
                          (((uint32)DpllConfigPtr->DpllCtrl4) << GTM_EIGHT_U);

    #if ( GTM_DPLL_NO_OF_ACTIONS > 24U )
    GTM_DPLL_CTRL_5.U = (unsigned_int)  \
                          (((uint32)DpllConfigPtr->DpllCtrl5) << GTM_EIGHT_U);
    #endif
    ...
    GTM_DPLL_IRQ_EN.U = (unsigned_int)  \
                        ((DpllConfigPtr->DpllIrqEnable) & GTM_GET_IRQ_VAL_32);
    GTM_DPLL_IRQ_MODE.U = (unsigned_int)  \
           ((DpllConfigPtr->DpllIrqEnable & GTM_GET_MODE_VAL_32) \
                                                        >> GTM_SHIFT_TO_LSB_32);
    GTM_DPLL_EIRQ_EN.U =  (unsigned_int)((DpllConfigPtr->DpllErrIntEnable &   \
...
IFX_LOCAL_INLINE void Gtm_lSpeConfig(void)
{
  ...
  for(Count = GTM_ZERO_U; Count < GTM_NO_SPE_MODULES; Count++)
  {
    ...
    if(SpeConfigPtr != NULL_PTR)
    {
      ...
      MODULE_GTM.SPE[Count].CNT.U = SpeConfigPtr->SpeRevCntr;
      MODULE_GTM.SPE[Count].CMP.U = SpeConfigPtr->SpeRevCmp;
      MODULE_GTM.SPE[Count].CTRL_STAT.U = SpeConfigPtr->SpeCtrlStat;
      MODULE_GTM.SPE[Count].IRQ_NOTIFY.U = GTM_SPE_CLEAR_INTERRUPTS;
      MODULE_GTM.SPE[Count].IRQ_MODE.U = (unsigned_int)  \
               (((uint32)SpeConfigPtr->SpeIrq & GTM_GET_MODE_VAL_8) >>   \
                        GTM_SHIFT_TO_LSB_8);
      MODULE_GTM.SPE[Count].IRQ_EN.U = (unsigned_int)  \
                        ((uint32)SpeConfigPtr->SpeIrq & GTM_GET_IRQ_VAL_8);
      MODULE_GTM.SPE[Count].EIRQ_EN.U = (unsigned_int)                       \
...
IFX_LOCAL_INLINE void Gtm_lCmpConfig(void)
{
  ...
  if(Gtm_kConfigPtr->GtmModuleConfigPtr->GtmCmpConfigPtr != NULL_PTR)
  {
    ...
    MODULE_GTM.CMP.IRQ_MODE.U = (unsigned_int)   \
    ...
    SRC_GTM_GTM0_CMPIRQ.U |= (unsigned_int)(GTM_BIT_SET << GTM_BIT_SRE);
    MODULE_GTM.CMP.EIRQ_EN.U = \
        (unsigned_int)(CmpConfigPtr->CmpErrIntEnable & GTM_CMU_GET_EIRQ_VAL_32);
  }
}
...
IFX_LOCAL_INLINE Std_ReturnType Gtm_lEnableGtm(void)
{
  ...
  #ifdef IFX_GTM_DEBUG
    GtmClockEnableStatus = (MODULE_GTM.CLC.B.DISS | TestGtm_DebugMask02);
  #else
    GtmClockEnableStatus = MODULE_GTM.CLC.B.DISS;
  #endif
  ...
  for(Count = GTM_ZERO_U; Count < GTM_NO_OF_ATOM_MODULES; Count++)
  {
    ...
    for(MinorCnt = GTM_ZERO_U; MinorCnt < GTM_SIXTEEN_U; MinorCnt++)
    {
      if((FupdBackupValue[Count][GTM_ZERO_U] & \
        ...
        GTM_ZERO_UL)
      {
        AtomAgcRegPtr->AGC_FUPD_CTRL.U |= (unsigned_int)\
      ...
      if((OutenCtrlBackupValue[Count][GTM_ZERO_U] & \
         ...
         GTM_ZERO_UL)
      {
        AtomAgcRegPtr->AGC_OUTEN_CTRL.U |=(unsigned_int)\
          (GTM_ATOM_CHAN_DISABLE << ((MinorCnt * GTM_ATOM_BITS_PER_CHAN) + \
          GTM_ONE_U));
      ...
      if((EndisCtrlBackupValue[Count][GTM_ZERO_U] \
           ...
           GTM_ZERO_UL)
      {
        AtomAgcRegPtr->AGC_ENDIS_CTRL.U |= (unsigned_int)\
          (GTM_ATOM_CHAN_DISABLE << ((MinorCnt * GTM_ATOM_BITS_PER_CHAN) + \
          GTM_ONE_U));
  ...
  else if(ReqType == GTM_READ_ARU_FRAME)
  {
    ...
    if (ReadReq == GTM_ISSUE_ARU_READ)
    {
      ...
      if((GTM_ARU_ACCESS.U & GTM_ARU_WREQ_RREQ_MASK) == 0UL)
      {
        ...
        GTM_ARU_ACCESS.U = AruAddress;
        GTM_ARU_ACCESS.U |= (unsigned_int)GTM_ARU_RREQ_MASK;
  ...
  if (TimerModule == GTM_TOM)
  {
    if(ChannelNo < GTM_TOM_TGC0_CHOSEN)
    {
      ((*(Ifx_GTM_TOMx*)(volatile void *)(MODULE_GTM.TOM)).TOM_TGC[ModuleNo]. \
        ...
        (unsigned_int)(ActionToPerform << (GTM_TOM_BITS_PER_CHAN * ChannelNo));
    }
...
Std_ReturnType Gtm_InitCheck (const Gtm_ConfigType* ConfigPtr)
{
  ...
  ErrorFlag = E_OK;
  TempClockEnable = MODULE_GTM.CLC.B.DISS;
