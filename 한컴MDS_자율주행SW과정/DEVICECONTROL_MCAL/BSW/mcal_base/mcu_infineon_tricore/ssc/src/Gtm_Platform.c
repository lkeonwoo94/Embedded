...
void Gtm_lResetGtmSRCReg(void)
{
  ...
  /* Clear Sticky bits of SRC register */
  SRC_GTMARUIRQ0.U |= GTM_CLEAR_STICKY_BITS;
  /* Disable SRE  */
  SRC_GTMARUIRQ0.B.SRE = GTM_ZERO_U;

  /* Clear Sticky bits of SRC register */
  SRC_GTMARUIRQ1.U |= GTM_CLEAR_STICKY_BITS;
  /* Disable SRE  */
  SRC_GTMARUIRQ1.B.SRE = GTM_ZERO_U;

  /* Clear Sticky bits of SRC register */
  SRC_GTMARUIRQ2.U |= GTM_CLEAR_STICKY_BITS;
  /* Disable SRE  */
  SRC_GTMARUIRQ2.B.SRE = GTM_ZERO_U;

  /* De-init FIFO SRC registers*/
  for(ChannelNo=GTM_ZERO_U; ChannelNo < GTM_NO_OF_FIFO_CHANNELS; ChannelNo++)
  {
    /* Clear Sticky bits of SRC register */
    MODULE_SRC.GTM.GTM[0].PSM[0][ChannelNo].U |= GTM_CLEAR_STICKY_BITS;
    /* Disable SRE */
    MODULE_SRC.GTM.GTM[0].PSM[0][ChannelNo].B.SRE = GTM_ZERO_U;
  }
  ...
  for(ModuleNo=GTM_ZERO_U; ModuleNo < GTM_NO_OF_TIM_MODULES; ModuleNo++)
  {
    ...
    {
      /* Clear Sticky bits of SRC register */
      MODULE_SRC.GTM.GTM[0].TIM[ModuleNo][ChannelNo].U |= GTM_CLEAR_STICKY_BITS;
    /* Disable SRE  */
      MODULE_SRC.GTM.GTM[0].TIM[ModuleNo][ChannelNo].B.SRE = GTM_ZERO_U;
    }
  }
  ...
  for(ModuleNo=GTM_ZERO_U; ModuleNo < GTM_NO_OF_TOM_MODULES; ModuleNo++)
  {
    ...
    {
      /* Clear Sticky bits of SRC register */
      MODULE_SRC.GTM.GTM[0].TOM[ModuleNo][ChannelNo].U |= GTM_CLEAR_STICKY_BITS;
      /* Disable SRE  */
      MODULE_SRC.GTM.GTM[0].TOM[ModuleNo][ChannelNo].B.SRE = GTM_ZERO_U;
    }
  }
  ...
  for(ModuleNo=GTM_ZERO_U; ModuleNo < GTM_NO_OF_ATOM_MODULES; ModuleNo++)
  {
                                                                 ...
                                                                 ChannelNo++)
    {
      ...
      /* Disable SRE  */
      MODULE_SRC.GTM.GTM[0].ATOM[ModuleNo][ChannelNo].B.SRE = GTM_ZERO_U;
    }
  }
  ...
  /* Clear Sticky bits of SRC register */
  MODULE_SRC.GTM.GTM[0].BRCIRQ.U |= GTM_CLEAR_STICKY_BITS;
  /* Disable SRE  */
  MODULE_SRC.GTM.GTM[0].BRCIRQ.B.SRE = GTM_ZERO_U;
  ...
  for(ModuleNo=GTM_ZERO_U; ModuleNo < GTM_NO_OF_MCS_MODULES; ModuleNo++)
  {
    ...
    {
      /* Clear Sticky bits of SRC register */
      MODULE_SRC.GTM.GTM[0].MCS[ModuleNo][ChannelNo].U |= GTM_CLEAR_STICKY_BITS;
      /* Disable SRE  */
      MODULE_SRC.GTM.GTM[0].MCS[ModuleNo][ChannelNo].B.SRE = GTM_ZERO_U;
    }
  }
  ...
  {
    /* Clear Sticky bits of SRC register */
    MODULE_SRC.GTM.GTM[0].DPLL[ModuleNo].U |= GTM_CLEAR_STICKY_BITS;
    /* Disable SRE  */
    MODULE_SRC.GTM.GTM[0].DPLL[ModuleNo].B.SRE = GTM_ZERO_U;
  }
  ...
  {
    /* Clear Sticky bits of SRC register */
    MODULE_SRC.GTM.GTM[0].SPEIRQ[ModuleNo].U |= GTM_CLEAR_STICKY_BITS;
    /* Disable SRE  */
    MODULE_SRC.GTM.GTM[0].SPEIRQ[ModuleNo].B.SRE = GTM_ZERO_U;
  }

  /* De-init CMP SRC registers */
  /* Clear Sticky bits of SRC register */
  SRC_GTM_GTM0_CMPIRQ.U |= GTM_CLEAR_STICKY_BITS;
    /* Disable SRE  */
  SRC_GTM_GTM0_CMPIRQ.B.SRE = GTM_ZERO_U;
       ...
       MinorCnt++)
    {
     ...
     PBConfigStructure and is within allowed range.*/
        ...
        if(TomChannelConfigPtr->TomUsage != GTM_DRIVER_COMPLEX)
        {
          ...
          (TomChannelConfigPtr->GtmTomControlWord)& (GTM_TOM_CLK_MASK);
        ...
        if(AtomChannelConfigPtr->GtmAtomUsage != GTM_DRIVER_COMPLEX)
        {
         ...
         Permitted for special function registers.*/
          AtomChannelRegPtr = &((*(Ifx_GTM_ATOMx*)(void *)
                       (MODULE_GTM.ATOM)).ATOM_CH[ModuleNo].CH[ChannelNo]);
          /*Set the CTRL value*/
          AtomChannelRegPtr->CTRL.U  |= (unsigned_int)                 \
          ...
          AtomAgcRegPtr->AGC.FUPD_CTRL.U |= (unsigned_int)\
